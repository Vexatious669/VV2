...
void TIMER::Initialize(bool localTimer)
{
	if (!IsInitialized)
	{
		Time = (*VV2::Pointers.IsSessionStarted && !localTimer) ? *VV2::Pointers.NetworkTime : *VV2::Pointers.GameTimer;
		IsInitialized = true;
	}
}
...
bool TIMER::HasTimePassed(int ms, bool localTimer)
{
	...
	int timePassed = (*VV2::Pointers.IsSessionStarted && !localTimer) ? (*VV2::Pointers.NetworkTime - Time) : (*VV2::Pointers.GameTimer - Time);
	if (timePassed >= ms)
		return true;
