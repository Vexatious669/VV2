...
namespace VV2::Lua
{
	...
	public:
		...
		{
			CreateObject<VV2::Vehicle>(state, luaL_checkinteger(state, 1));
			return 1;
		}

		static int Create(lua_State* state)
		{
			CopyObject<VV2::Vehicle>(state, VV2::Vehicle::Create(GetHashArgument(state, 1), GetObject<rage::fvector3>(state, 2), lua_gettop(state) >= 3 ? luaL_checknumber(state, 3) : 0.0f));
			return 1;
		}

		static int Fix(lua_State* state)
		{
			GetObject<VV2::Vehicle>(state, 1).Fix();
			return 0;
		}

		static int GetGear(lua_State* state)
		{
			lua_pushinteger(state, GetObject<VV2::Vehicle>(state, 1).GetGear());
			return 1;
		}

		static int GetRevRatio(lua_State* state)
		{
			lua_pushnumber(state, GetObject<VV2::Vehicle>(state, 1).GetRevRatio());
			return 1;
		}

		static int GetSpeed(lua_State* state)
		{
			lua_pushnumber(state, GetObject<VV2::Vehicle>(state, 1).GetSpeed());
			return 1;
		}

		static int Upgrade(lua_State* state)
		{
			GetObject<VV2::Vehicle>(state, 1).Upgrade();
			return 0;
		}

		static int GetPlateText(lua_State* state)
		{
			auto text = GetObject<VV2::Vehicle>(state, 1).GetPlateText();
		...
		{
			GetObject<VV2::Vehicle>(state, 1).SetPlateText(CheckStringSafe(state, 2));
			return 0;
		}
		...
		{
			lua_pushboolean(state, GetObject<VV2::Vehicle>(state, 1).IsSeatFree(luaL_checkinteger(state, 2)));
			return 1;
		}

		static int SupportsBoost(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Vehicle>(state, 1).SupportsBoost());
			return 1;
		}

		static int IsBoostActive(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Vehicle>(state, 1).IsBoostActive());
			return 1;
		}

		static int SetBoostCharge(lua_State* state)
		{
			GetObject<VV2::Vehicle>(state, 1).SetBoostCharge(lua_gettop(state) >= 2 ? luaL_checkinteger(state, 3) : 100);
			return 0;
		}

		static int LowerStance(lua_State* state)
		{
			GetObject<VV2::Vehicle>(state, 1).LowerStance(CheckBooleanSafe(state, 2));
			return 0;
		}

		static int BringToHalt(lua_State* state)
		{
			GetObject<VV2::Vehicle>(state, 1).BringToHalt(luaL_checknumber(state, 2), luaL_checkinteger(state, 3));
			return 0;
		}

		static int SetOnGroundProperly(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Vehicle>(state, 1).SetOnGroundProperly());
			return 1;
		}

		static int GetFullName(lua_State* state)
		{
			auto name = GetObject<VV2::Vehicle>(state, 1).GetFullName();
		...
		virtual void Register(lua_State* state) override
		{
			...
			Metatable<VV2::Vehicle>::Register(state);
			Metatable<VV2::Entity>::AddSubclass<VV2::Vehicle>();
