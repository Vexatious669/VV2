...
namespace VV2::Lua
{
	...
	public:
		...
		{
			CreateObject<VV2::Entity>(state, luaL_checkinteger(state, 1));
			return 1;
		}

		static int GetHandle(lua_State* state)
		{
			lua_pushinteger(state, GetObject<VV2::Entity>(state, 1).GetHandle());
			return 1;
		}

		static int IsValid(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsValid());
			return 1;
		}

		static int IsPed(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsPed());
			return 1;
		}

		static int IsVehicle(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsVehicle());
			return 1;
		}

		static int IsObject(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsObject());
			return 1;
		}

		static int IsPlayer(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsPlayer());
			return 1;
		}

		static int IsMissionEntity(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsMissionEntity());
			return 1;
		}

		static int GetModel(lua_State* state)
		{
			lua_pushinteger(state, (int)GetObject<VV2::Entity>(state, 1).GetModel());
			return 1;
		}

		static int GetPosition(lua_State* state)
		{
			CopyObject<rage::fvector3>(state, GetObject<VV2::Entity>(state, 1).GetPosition());
			return 1;
		}

		static int GetRotation(lua_State* state)
		{
			CopyObject<rage::fvector3>(state, GetObject<VV2::Entity>(state, 1).GetRotation(lua_gettop(state) >= 2 ? luaL_checkinteger(state, 2) : 2));
			return 1;
		}

		static int GetVelocity(lua_State* state)
		{
			CopyObject<rage::fvector3>(state, GetObject<VV2::Entity>(state, 1).GetVelocity());
			return 1;
		}

		static int SetPosition(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetPosition(GetObject<rage::fvector3>(state, 2));
			return 0;
		}

		static int SetRotation(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetRotation(GetObject<rage::fvector3>(state, 2), lua_gettop(state) >= 3 ? luaL_checkinteger(state, 3) : 2);
			return 0;
		}

		static int SetVelocity(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetVelocity(GetObject<rage::fvector3>(state, 2));
			return 0;
		}

		static int GetHeading(lua_State* state)
		{
			lua_pushnumber(state, GetObject<VV2::Entity>(state, 1).GetHeading());
			return 1;
		}

		static int SetHeading(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetHeading(luaL_checknumber(state, 2));
			return 0;
		}

		static int GetSpeed(lua_State* state)
		{
			lua_pushnumber(state, GetObject<VV2::Entity>(state, 1).GetSpeed());
			return 1;
		}

		static int SetCollision(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetCollision(lua_toboolean(state, 2));
			return 0;
		}

		static int SetFrozen(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetFrozen(lua_toboolean(state, 2));
			return 0;
		}

		static int Delete(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).Delete();
			return 0;
		}
		...
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsNetworked());
			return 1;
		}

		static int IsRemote(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsRemote());
			return 1;
		}

		static int HasControl(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).HasControl());
			return 1;
		}
		...
		{
			lua_pushinteger(state, GetObject<VV2::Entity>(state, 1).GetNetworkObjectId());
			return 1;
		}

		static int PreventMigration(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).PreventMigration();
			return 0;
		}

		static int ForceControl(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).ForceControl();
			return 0;
		}

		static int RequestControl(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).RequestControl(lua_gettop(state) >= 2 ? luaL_checkinteger(state, 2) : 100);
			return 0;
		}
		...
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsInvincible());
			return 1;
		}

		static int SetInvincible(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetInvincible(lua_toboolean(state, 2));
			return 0;
		}

		static int IsDead(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsDead());
			return 1;
		}

		static int Kill(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).Kill();
			return 0;
		}

		static int GetHealth(lua_State* state)
		{
			lua_pushinteger(state, GetObject<VV2::Entity>(state, 1).GetHealth());
			return 1;
		}

		static int SetHealth(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetHealth(luaL_checkinteger(state, 2));
			return 0;
		}

		static int GetMaxHealth(lua_State* state)
		{
			lua_pushinteger(state, GetObject<VV2::Entity>(state, 1).GetMaxHealth());
			return 1;
		}

		static int IsVisible(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).IsVisible());
			return 1;
		}

		static int SetVisible(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetVisible(CheckBooleanSafe(state, 2));
			return 0;
		}

		static int GetAlpha(lua_State* state)
		{
			lua_pushinteger(state, GetObject<VV2::Entity>(state, 1).GetAlpha());
			return 1;
		}

		static int SetAlpha(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).SetAlpha(luaL_checkinteger(state, 2));
			return 0;
		}

		static int ResetAlpha(lua_State* state)
		{
			GetObject<VV2::Entity>(state, 1).ResetAlpha();
			return 0;
		}

		static int HasInterior(lua_State* state)
		{
			lua_pushboolean(state, GetObject<VV2::Entity>(state, 1).HasInterior());
			return 1;
		}
		...
		virtual void Register(lua_State* state) override
		{
			...
			Metatable<VV2::Entity>::Register(state);
